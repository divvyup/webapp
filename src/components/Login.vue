<template>
  <div id="login" style="height: 100%;">
    <section class="thinmessage">
      <b-message title="Note!" type="is-info" :active.sync="isActive">
        This site is currently in demo mode. Please login with the following credentials. Userame: 'user', Password: 'password'
      </b-message>
    </section>
    <h1><strong>Login to your account!</strong><hr class="centered"></h1>
    ...
    <a class="button is-info is-outlined" v-on:click="login()">Login!</a>
  </div>
</template>
...
export default {
  ...
  methods: {
    login: function () {
      this.$http.post(API + '/api/v1/auth/login', {
